...
exports.init = function (app) {
  ...
  /* I WANT TO USE THESE IN THE FUTURE FOR GITHUB!!!!*/
  //app.use(route.get('/login/github', githubLogin));
  //app.use(route.get('/login/github/callback', githubCallback));
...
async function login(ctx) {
  ...
  var user = await mongo.users.findOne({email: credentials.email}, {email: 1, username: 1, phone: 1});

  if (!user) {
    ctx.throw(401, 'Incorrect entry!');
